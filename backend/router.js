...
function registerRecommend() {
  router.get('/api/music/getRecommend', async (ctx) => {
    // 第三方服务接口 url
    ...
    // 计算签名值
    const sign = getSecuritySign(reqData)
...
// 注册歌手列表接口路由
function registerSingerList() {
  router.get('/api/music/getSingerList', async (ctx) => {
    const url = 'https://u.y.qq.com/cgi-bin/musics.fcg'
    const HOT_NAME = '热'
...
// 注册歌手详情接口路由
function registerSingerDetail() {
  router.get('/api/music/getSingerDetail', async (ctx) => {
    const url = 'https://u.y.qq.com/cgi-bin/musics.fcg'
    const reqData = JSON.stringify({
      comm: { ct: 24, cv: 0 },
      singerSongList: {
...
function registerSongsUrl() {
  router.get('/api/music/getSongsUrl', async (ctx) => {
    ...
    function process(mid) {
      ...
      const sign = getSecuritySign(JSON.stringify(reqData))
      const url = `https://u.y.qq.com/cgi-bin/musics.fcg?_=${getRandomVal()}&sign=${sign}`

      // 发送 post 请求
      return post(url, reqData).then((response) => {
        const data = response.data
        if (data.code === ERR_OK) {
...
// 注册歌词接口
function registerLyric() {
  router.get('/api/music/getLyric', async (ctx) => {
...
function registerAlbum() {
  router.get('/api/music/getAlbum', async (ctx) => {
    const req = ctx.request
    const reqData = {
      req_0: {
    ...
    const response = await post(url, reqData)
    const data = response.data
    if (data.code === ERR_OK) {
...
// 注册排行榜接口
function registerTopList() {
  router.get('/api/music/getTopList', async (ctx) => {
    const url = 'https://u.y.qq.com/cgi-bin/musics.fcg'
...
// 注册排行榜详情接口
function registerTopDetail() {
  router.get('/api/music/getTopDetail', async (ctx) => {
    const req = ctx.request
    const url = 'https://u.y.qq.com/cgi-bin/musics.fcg'
...
function registerHotKeys() {
  router.get('/api/music/getHotKeys', async (ctx) => {
...
function registerSearch() {
  router.get('/api/music/search', async (ctx) => {
    ...
    const { query, page, showSinger } = ctx.request.query
    ...
    try {
      const response = await get(url, reqData)
      const data = response.data
      if (data.code === ERR_OK) {
        const songList = []
